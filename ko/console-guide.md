...
```
RCS Bizmessage 서비스를 이용하려면 [콘솔 > RCS Bizmessage > 본인 인증]에서 본인 인증을 사전에 진행한 후 사용할 수 있습니다(전기통신사업법 관련 고시 준수).
```
...
* 최초 가입ID가 관리자, 이후 가입ID가 담당자가 됩니다. 관리자는 브랜드 개설이 가능합니다.(관리자 변경 가능)
* RCS 브랜드는 기업프로필입니다. 브랜드를 개설하고 승인 요청합니다.
* RCS 브랜드의 대표발신번호를 설정합니다. 이 후 등록된 번호로 대표번호 변경이 가능합니다.<br/>
통신서비스 가입증명원 첨부시, RCS 브랜드 사업자번호와 통신서비스 가입증명원에 명시된 사업자번호가 ‘일치＇해야 합니다.입력한 발신번호의 모든 통신서비스 가입증명원을 첨부해야 합니다.
* RCS 담당자 승인하며 영업일 2~3일 소요됩니다.
* 브랜드 개설 상단 ‘브랜드 가이드’를 클릭하면 관련 가이드를 참고할 수 있습니다.

### 브랜드 대행사 설정
RCS 브랜드 승인 완료 후, 대행사를 ‘㈜ NHN Cloud’로 설정합니다.

* RCS Biz Center [서비스관리 > 브랜드 > 하단 브랜드 운영 현황 > 브랜드 선택 > 운영관리] 메뉴로 이동하여 대행사를 지정합니다.
* 대행사 권한 추가 버튼을 클릭하여, 대행사명에 ㈜ NHN Cloud 를 검색 및 선택합니다.
...
* RCS Biz Center [서비스관리 > 브랜드 > 하단 브랜드 운영 현황 > 브랜드 선택 > 발신번호 관리] 메뉴로 이동하여 발신번호를  등록합니다.
...
* RCS Biz Center [서비스관리 > 브랜드 > 템플릿 관리] 메뉴에서 템플릿을 등록합니다.
...
* 템플릿 서술형(description)은 문자메시지처럼 문장 형태의 텍스트를 입력할 수 있습니다.
* 템플릿 스타일(cell)은 행, 단을 나누어 입력할 수 있습니다. 볼드, 글자크기, 정렬, 글자색을 설정하고 라인을 추가할 수 있습니다.
...
4. [발송 관리]메뉴에서 SMS 이용 설정을 진행합니다.
5. 발송 유형을 **SMS**로 선택합니다.
6. **메시지 내용(최대 100자), 버튼(최대 1개)을 추가합니다.**
...
3. 발신 내용이 광고인지 아닌지 선택합니다. 광고인 경우 080 수신거부 번호를 선택해야 합니다.
4. [발송 관리]메뉴에서 SMS 이용 설정을 진행합니다.
5. 발송 유형을 **LMS**로 선택합니다.
6. **메시지 제목(최대 30자), 내용(최대 1300자), 버튼(최대 3개)을 추가합니다.**
7. 우측의 수신자 설정에서 발송할 수신 번호를 작성하고 추가 버튼을 클릭합니다(최대 50명).
8. 발송 버튼을 클릭합니다.
...
3. 발신 내용이 광고인지 아닌지 선택합니다. 광고인 경우 080 수신거부 번호를 선택해야 합니다.
4. [발송 관리]메뉴에서 SMS 이용 설정을 진행합니다.
5. 발송 유형을 **MMS**로 선택합니다.
6. 상세 선택에서 **가로형 또는 세로형**을 선택합니다.
7. **카드 별로 제목(최대 30자), 내용(최대 1300자), 이미지(최대 1개), 버튼(카드당 최대 2개)을 추가합니다.**
...
4. [발송 관리]메뉴에서 SMS 이용 설정을 진행합니다.
5. 발송 유형을 **MMS**로 선택합니다.
6. 상세 선택에서 **슬라이드형과 슬라이드 크기를 선택**합니다.
7. 메시지의 슬라이드 개수 설정합니다(최소 3개, 최대 6개).
8. **카드 별로 제목(최대 30자), 내용(최대 30자), 이미지(최대 1개), 버튼(최대 2개)을 입력합니다.**
...
4. 발송할 템플릿을 선택합니다.
5. 템플릿은 사전에 RCS Biz Center에서 등록합니다.
6. 프리 템플릿인 경우에는 보낼 메시지(최대 90자)를 작성합니다.
7. 우측의 수신자 설정에서 발송할 수신 번호를 작성하고 추가 버튼을 클릭합니다(최대 50명).
8. 수신자 별로 **템플릿 치환자에 맞는 값을 입력**합니다.
9. 발송 버튼을 클릭합니다.

### 버튼 유형
메시지 하단에 버튼 기능을 활용할 수 있는 버튼입니다.
...
현재 위치 공유
* 수신자가 발신자에게 수신자의 현재 위치를 메시지 전송합니다.
* 버튼명을 입력합니다.

URL 연결
* 웹링크로 연결됩니다.
* 버튼명을 입력 후 버튼 클릭시 연결할 링크를 입력합니다.
* 링크 입력 시 ‘http://', 'https://‘ 필수 입력해야 합니다.
...
* 080 수신거부 번호 가입 방법은 [080 수신거부 설정](https://docs.nhncloud.com/ko/Notification/SMS/ko/console-guide/#080)에서 확인 가능합니다.
